...
export const registerDataTools: RegisterFn = (server, rt) => {
      ...
      try {
        return ok(
          await runTool("risk_size", rt.opts, { direction, risk_usd, risk_pct, stop_loss_pct, trail_pct, target_r_multiple, chain, token, quote, strategy }, chain, async () => {
            ...
            const priceAddr = /^0x[0-9a-fA-F]{40}$/.test(resolved) ? resolved : profile.weth;
            const priceUsd = await getCurrentPrice(priceAddr, rt.opts.logger).catch(() => null);
            // v105.1: price the QUOTE token (default USDC ≈ $1) so finalUsd
            // converts to the quoteAmount the buy next-action will spend.
            const quoteSym = quote ?? "USDC";
            const quoteResolved = resolveToken(profile, quoteSym);
            const quotePriceUsd = quoteResolved
              ? await getCurrentPrice(quoteResolved, rt.opts.logger).catch(() => null)
              : null;
  ...
  server.tool(
    ...
    "v65: open-position review for EXIT timing. For each currently-held position returns cost basis, current value, unrealized P&L (abs + %), the weighted-average acquisition date, holding days, and — the actionable tax signal — projectedTerm ('short' / 'long' / 'untracked' if sold NOW) + daysToLongTerm (how long until short-term flips to long-term). Plus a summary { totalCostBasisQuote, totalValueQuote, totalUnrealizedQuote, unpricedCount, approachingLongTerm (short-term positions within 30d of long-term) }. Use to decide WHEN to exit: 'WETH is 340d held, 25d to long-term rates — wait' is a concrete decision this surfaces and nothing else did. The exit counterpart to price_context (entry timing) and the open-position completion of the gains report's realized holding period. Runs the same cost-basis walker every P&L surface shares; the strategy tag is stripped so positions are PORTFOLIO-level (total per (chain,token)) unless `strategy` scopes it. Deterministic given live marks. mode='real' (success trades) default; 'paper' reviews the virtual book. Errors: none typical (empty → no positions).",
    {
      ...
      withProtection: z.boolean().optional().describe("v110: annotate each position with its downside-protection status — protection { status (protected/partial/unprotected), protectedAmount, unprotectedAmount, stops[] (active trailing/stop-loss sells covering it), downsideToStopPct (% cushion above the highest fixed stop floor before it fires) }. Merges the v76 protection audit into the position review so 'how's it doing AND is it protected?' is ONE call — surfaces a winner riding UNPROTECTED. v119: also returns top-level recommendedActions[] — a ready `order_create` trailing-stop (covering the uncovered amount) per UNPROTECTED/PARTIAL position, so the agent protects in one step instead of hand-building the order. Off by default (loads active orders)."),
    },
    async ({ mode, account, chain, strategy, withContext, contextDays, withProtection }) => {
      try {
        return ok(
          await runTool("open_positions", rt.opts, { mode, account, chain, strategy, withContext, contextDays, withProtection }, chain, async () => {
            const { gatherOpenPositions } = await import("../openPositions.js");
            return { ok: true, ...(await gatherOpenPositions({ mode: mode ?? "real", account, chain, strategy, withContext, contextDays, withProtection, config: rt.getConfig() })) };
          }),
        );
      } catch (e) {
        return fail(toToolError(e));
  ...
  server.tool(
    "strategy_compare",
    "v83: RANK strategies by realized performance for capital allocation — scale the winners, cut the bleeders. Returns { ok, strategies: [{ strategy, realizedUsd, tradeCount, closes, wins, losses, winRatePct, volumeUsd, avgRealizedPerClose, avgWinUsd, avgLossUsd, profitFactor, payoffRatio, lastTradeAt }] (ranked realized desc), totalRealizedUsd, best, worst, bleeding[] (negative-realized strategies), unpricedTrades, summary }. v114 EDGE metrics answer 'does this strategy actually have an edge?' — which win rate alone CANNOT: profitFactor (grossWin/grossLoss; >1 = profitable, magnitude = cushion; null when no losses), payoffRatio (avgWin/avgLoss size asymmetry), avgWinUsd/avgLossUsd, and avgRealizedPerClose (= per-trade expectancy in $). A 70%-win-rate strategy with profitFactor < 1 (big losers, small winners) is bleeding despite the win rate — these surface it. The effectiveness counterpart to strategies_list (discovery) + strategy_report (one-strategy deep-dive): this is the side-by-side comparison that answers 'where do I put more capital?'. WIN RATE matters as much as total P&L — +$100 over 2 lucky trades vs 50 steady ones are very different bets, and this surfaces both. Deterministic + offline: uses the shared cost-basis reducer (same core every P&L surface shares post-v82, so it can't diverge) + a stablecoin-$1 quote model — no marks, no RPC, same answer every run. Realized-only (closed-trade bottom line); non-stablecoin-quoted trades are excluded from P&L (unpricedTrades) since they can't be valued deterministically — run `pnl` for live-priced unrealized + gas. mode='real' (default, success trades) or 'paper'. `days` scopes a lookback window.",
    {
