...
export const registerStrategyTools: RegisterFn = (server, rt) => {
  ...
  server.tool(
    "playbook_outcome",
    "v50: 'did promoting this strategy deliver what the paper run promised?' — the BACKWARD half of the trust pipeline (playbook_promote_check is the forward half). Compares the frozen paper baseline (paper_trades, which stopped growing at promotion) against the live fills (trades) for the same strategy tag, NORMALIZED per-fill and per-week so a 50-fill paper run and a 6-fill live run compare fairly. Both eras run through the SAME cost-basis walker (computePaperPnlMtm) → apples-to-apples realized PnL. Verdict: on_track | underperforming | diverged | insufficient_data. diverged = paper realized > 0 but live fills realize ≤ 0/fill (the strategy is not making money with real execution). underperforming = live per-fill realized < 60% of paper, OR live median slippage > 1.5× the paper assumption, OR live cadence < 50% of paper (with ≥2 live days), OR (v122) live profit factor < 60% of paper's when both eras have ≥5 closes and paper's edge was genuinely positive (≥1.2) — the edge SHAPE degraded. insufficient_data = no live fills, < 3 live fills, or no paper baseline. v122: each era also carries `edge` (profit factor / payoff / win rate / expectancy via the SAME computeEdge every other trust gate uses) and `comparison.profitFactorRatioPct` — the per-fill check catches a falling AVERAGE; the profit-factor check catches a collapsing CUSHION the average hides (paper 3.0 → live 1.1 = one bad trade from bleeding even while per-fill still reads positive). Deterministic + offline: the verdict keys off realized PnL (closed round-trips), the live fills' own realized slippage + gas, and cadence — never off unrealized marks. reasons[] names every flag. v98 `recommendedActions[]` closes the detect→respond loop with the structured protective response: diverged → demote NOW (playbook_promote {to:'paper', yes:true} — halts the real-money bleed while preserving HWM/counters); underperforming → consider demoting (playbook_promote {to:'paper'}, yes intentionally NOT pre-filled — recoverable-or-cut is your call); on_track / insufficient_data carry none.",
    {
  ...
  server.tool(
    ...
    "Flip a deployed playbook between paper and real trading IN PLACE — the dry-run loop's graduation step. Every live primitive (active orders; active+paused schedules/rebalance plans) routes through the same edit machinery as order_edit/schedule_edit/rebalance_edit, so trailing HWM water marks, run counters, and drift telemetry ALL survive: a trailing stop that tracked a $3,500 HWM in paper keeps protecting from $3,500 the moment it's real. Symmetric: to='paper' demotes a live strategy back to the sandbox without losing state. Rows already in the target mode (or terminal) are reported in skipped[] with reasons; alreadyInTarget=true means nothing flipped. v36: promotes to real run an as-if-real funding PREFLIGHT (the runway machinery with paper primitives bucketed as real — spend tokens AND gas vs the actual wallet); the result's preflight.warnings list findings worst-first and requireFunded=true aborts with INSUFFICIENT_BALANCE when the wallet cannot fund even one fire. Preflight is advisory by default and best-effort (a dead RPC warns, never blocks; skipPreflight disables). v52: promotes to real ALSO attach a `safety` posture (the safety_review audit) — funding asks 'can the wallet PAY?', safety asks 'is it GUARDED?'; requireSafe=true aborts with SAFEGUARD_TRIGGERED on a CRITICAL guardrail gap (safety disabled, or no per-tx AND no daily USD ceiling — unbounded real trading). v96: promotes to real ALSO attach a `readiness` report (the playbook_promote_check audit) — the third question, 'is the STRATEGY any good?'; requireReady=true aborts with PROMOTE_NOT_READY when the strategy hasn't proven itself on paper (not_ready: runtime < 7d, or < 5 fills). All three gates are advisory by default and best-effort (a dead RPC warns, never blocks); requireFunded / requireSafe / requireReady make them hard stops — all STRONGLY recommended for agent-driven promotes (together they stop an unproven, unaffordable, or unguarded strategy reaching real funds). skipPreflight disables all three. Destructive direction (to real fires actual trades from the next engine tick) requires `yes: true`. Errors: INVALID_PARAMS (id not found, not deployed, no owned primitives, yes missing).",
    {
      ...
      requireReady: z.boolean().optional().describe("v96: abort (PROMOTE_NOT_READY) when the strategy hasn't PROVEN itself on paper — the readiness check (playbook_promote_check) returns not_ready on the hard evidence floors (paper runtime < 7d, or < 5 fills). Funding asks 'can it PAY?', safety 'is it GUARDED?', this asks 'is the STRATEGY any good?'. STRONGLY recommended for agent-driven promotes — it stops an unproven strategy (1 fill, 1 day) graduating to real funds. caution-level quality flags never block. The result always includes the `readiness` report regardless."),
      skipPreflight: z.boolean().optional().describe("Skip ALL THREE preflights (funding, safety-posture, AND readiness) entirely (RPC-less environments). The result then has preflight: null, safety: null, readiness: null."),
    },
    async ({ id, to, yes, requireFunded, requireSafe, requireReady, skipPreflight }) => {
      try {
        return ok(
          await runTool("playbook_promote", rt.opts, { id, to, yes, requireFunded, requireSafe, requireReady, skipPreflight }, undefined, async () => {
            if (yes !== true) {
              throw new ToolError("INVALID_PARAMS", `Confirmation flag required: pass yes=true.`);
            }
            ...
            // v96 readiness gate: funding asks "can it PAY?", safety "is it
            // GUARDED?"; this asks "has the STRATEGY proven itself on paper?".
            // Always attached for the agent to inspect; requireReady gates on
            // a not_ready verdict (hard evidence floors). A gather failure
            // (RPC down) degrades to advisory; the not_ready block propagates.
            let readiness: import("../promoteCheck.js").PromoteCheckReport | null = null;
            if (to === "real" && skipPreflight !== true) {
              const { gatherPromoteCheck, promoteReadinessBlocker } = await import("../promoteCheck.js");
              try {
                readiness = await gatherPromoteCheck({ playbookId: id, config: rt.getConfig() });
              } catch { /* readiness gather unavailable (RPC) — advisory, proceed */ }
              if (readiness && requireReady === true) {
                const blocker = promoteReadinessBlocker(readiness);
                if (blocker) throw new ToolError("PROMOTE_NOT_READY", `Promote aborted — ${blocker}`);
              }
            }
            const result = promotePlaybook({ playbookId: id, to });
            return { ok: true, ...result, preflight, safety, readiness };
          }),
        );
      } catch (e) {
        return fail(toToolError(e));
