...
export const registerSecurityTools: RegisterFn = (server, rt) => {
  ...
  server.tool(
    "safety_headroom",
    "v53: runtime safety headroom — 'how much room do I have left across every active limit, and what's the binding constraint right now?'. Call this BEFORE sizing a trade so you stay inside the envelope instead of bouncing off a rejection. Returns entries[] (one per active limit: daily USD cap utilization, per-strategy budgets lifetime+24h, drawdown distance-to-trip, per-strategy realized-LOSS breaker distance-to-trip (v99 — one entry per losing strategy; profitable strategies have full headroom and are omitted), trade rate-limit readiness, net-exposure position caps, per-tx ceiling) each with used / remaining / utilizationPct + status (ok | approaching ≥80% | exhausted ≥100% | tripped), plus `binding` (the tightest active constraint) and counts. v99: the loss breaker was previously the only limit with no early warning — a strategy stayed silent until a buy was blocked; now an agent sees the trip approaching and can stop buying that strategy first. Where v51 safety_review shows what's CONFIGURED, this shows how much is LEFT. Scope: account + chain default to the active ones; pass account/chain to scope the daily-USD + rate-limit reads. Deterministic + offline (config + trades/drawdown tables; no oracle, no RPC).",
    {
