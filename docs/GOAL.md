...
**Phase 126 — paper 累计 USD 护栏对齐 live：daily-USD cap + strategy budget 现已强制（按 value_usd 计价）；补全 v125 刻意推迟的最后两块（close the deferred paper cumulative-USD parity gap — paper now enforces the daily-USD cap + strategy budget, valued correctly off value_usd）** ✅
- **背景/盲点**：v125 把单笔 USD 上限、滑点、白/黑名单、亏损断路器对齐了 live，但 paperTrade.ts 的注释明确写明两个**累计**窗口被"刻意推迟"——daily-USD cap 与 strategy budget——唯一根因是 paper_trades **缺 value_usd 列**，无法对非稳定币 quote 正确计价（呼应 v104-v111 的 value_usd 纪律：WETH-quote 的裸 quote_amount 会把美元额低估 ~1000×）。于是一个 paper 策略能在 live 早该被每日上限/策略预算拦下的地方继续成交，paper **高估**了策略能部署的资本量与持续时长，promote-outcome 反向闸（v122）便会把 paper/live 的发散误判为策略本身的优劣
- 为什么重要：直接服务最核心目标——信任管道（backtest→paper→promote→live）的 paper-fidelity。这不是加 feature，而是补全"dry-run 必须在 live 会拒的地方同样拒"这一 v125 已立项不变量的最后两块；同时是 value_usd 正确性纪律向 paper 路径的对称延伸，而非 stablecoin-only 近似的捷径
- 实现（最大化复用 + 单一来源纪律）：① DB 迁移追加 `ALTER TABLE paper_trades ADD COLUMN value_usd REAL`（与 v65 给 trades 加列同模式，自动顺序迁移；NULL=legacy/unpriceable）。② recordPaperTrade 落库 paperEstimatedUsd（baseUsd × baseAmount，trade-time USD，quote-token 无关；不可用落 null，与 live trade.ts 的 estimatedUsd ?? null 对称）。③ 把 live 与 paper 四个累计窗口的求和折叠成单一 helper `sumUsdRows`（value_usd ?? quote_amount，跳过 NaN），dailyUsdVolume/usdSpentUnderStrategy 与新增的 `paperDailyUsdVolume`/`paperUsdSpentUnderStrategy` 共用同一份计价约定（paper 无 status → 全部计入，paper fill 即成交）。④ 把 live 的每日上限抽成 `enforceDailyUsdLimit(estimatedUsd, config, volumeLookup)`，**live 与 paper 共用同一份强制定义**——只换 volumeLookup；strategy budget 直接复用 enforceStrategyBudget 已有的 spentLookup 注入缝喂 paperUsdSpentUnderStrategy，零执行逻辑重复。⑤ executePaperTrade 紧跟 v125 时序（per-tx cap 之后）接线两个护栏。明确仍**不**强制：合约白名单（paper 无 router）、回撤断路器（需 paper-equity 接线）、仓位上限（已由 v38 强制）
- 测试覆盖：+13——paperTrade(+8)：value_usd 落库≈baseUsd×baseAmount、legacy/backfill 行落 NULL 且按 quote_amount 回退、daily cap 恰好==上限放行/超额拒(AMOUNT_EXCEEDS_LIMIT 与 live 一致)、**核心回归守卫**：WETH-quote 买入按 USD 计价而非裸 quote_amount（$4000 上限拒、$6000 放行且 value_usd≈$5000）、strategy budget per-fire/lifetime 命中拒、无 tag 放行、parity 对照锁死 dry-run 与 live enforceDailyUsdLimit 在同一数字上同拒；db(+5)：sumUsdRows value_usd 优先/quote 回退/跳 NaN、paperDailyUsdVolume 按 USD 计价+chain 过滤、paperUsdSpentUnderStrategy lifetime vs 24h 窗口；3650 测试全绿（+14）
...
- 实现（最大化复用，几乎零新逻辑）：combineRiskPosture 早已**接受** `protection` 入参（只是一直被喂 null）。让 orchestrator（CLI health + MCP health）best-effort 调 gatherPositionProtection（与 headroom 同契约：需 live marks + active orders，失败降级为 {error}），把 ProtectionSummaryLite 传进纯函数 composeHealthReport。compose 把它喂给 combineRiskPosture（自动产出 `unprotected_exposure` concern code，阈值沿用 ≥50% 书面价值无保护），并据此把 notChecked 的 "protection" 去掉；deriveNextActions 新增 `unprotected_positions` 规则，**完全照搬** concentration_high/mev_exposed 的"读 risk.concernCodes → emit action"写法（severity high，命令指向 `tradekit positions --protection`）。MCP health 工具描述同步（删掉"protection 在 notChecked、去跑 risk_posture"的过期说法）
- 关键决策：保护审计走整本书（不按 dashboard 的 account/chain 收窄）——安全信号宁可更广；frac 用保护报告自身的 totalValueUsd 算，内部自洽、与 dashboard scope 无错配。来源用 gatherPositionProtection（基于**交易出来的**持仓、天然排除稳定币 quote 噪声），而非原始钱包余额
- 测试覆盖：+4——≥50% 无保护→risk 出 unprotected_exposure concern + notChecked 不再含 protection + 行动清单出 unprotected_positions(high) 且命令含 positions --protection；全保护→已评估但无 concern 无 action；<50% 小敞口→不打扰清单；protection fetch 失败→维持 notChecked + protection_failed section error 且 dashboard 仍正常组装；3628 测试全绿（+4）
