...
**Multi-pair bundles (v43).** The v1 same-pair constraint is lifted: a playbook mixing ETH/USDC and WBTC/USDC strategies backtests as one bundle. The CLI/MCP fetch **one price series per unique base** (max 6) and the simulator walks a **merged timeline** — the sorted union of every series' timestamps, each strategy evaluating and filling at its own base's at-or-before price (the same convention the rebalance backtest uses for misaligned series). Everything trades out of the ONE shared quote balance, so cross-base capital interactions are real: an ETH DCA that drains USDC starves the WBTC breakout entry, OCO groups cascade across bases, and valuation/hold/risk-metrics value each base at its own series' prices. Two invariants stay: **one shared quote per bundle** (shared-balance accounting is ambiguous otherwise) and **hook legs trade their parent strategy's pair** (they're sized from the parent's fill). Rebalance plans remain excluded — `backtest rebalance` is their own multi-asset simulator. The validator names every violation in one message.
...
**Promote readiness check (v49).** `tradekit playbook promote-check <id>` answers the question every analytics investment in this tool ultimately serves: **"is this paper strategy ready for real money?"** It composes four sections into one deterministic verdict: **runtime evidence** (floors: 7 days + 5 fills — below either is `NOT READY` outright; not enough market regimes or execution evidence), **paper performance** (realized + mark-to-market via the established cost-basis walker, scaled to $/month at the observed cadence), the **v48 paper-book risk block** (max drawdown / vol / Sharpe — book-level, and the report says so), and the **friction-reality cross-check**: paper fills record the slippage they *assumed* (`slippage_bps`); your real trades record what fills actually *cost* (v44's realized slippage + gas stats). Projecting the real numbers onto the paper cadence yields a monthly friction estimate and its share of paper PnL — above 50% flags `CAUTION: the edge may not survive real execution`, and a paper run that assumed less slippage than your real fills average gets called optimistic by name. Missing data degrades honestly (no real history → nulls, not false confidence; no paper snapshots → a pointer at the snapshot worker, never a block). Agents get `playbook_promote_check`; `playbook promote` itself still runs the funding preflight — this is the strategy-quality half of the decision.

**Paper equity parity (v48).** The paper book — the on-ramp where strategies prove themselves before `promote` — now feeds the same equity stack. The snapshot worker (and manual `tradekit portfolio snapshot --paper`) values the virtual book at live prices and writes to the SAME `portfolio_snapshots` table under a namespaced `paper:<account>` scope, so the **entire** stack works on paper strategies with zero further wiring: `tradekit equity --accounts-key "paper:default"` renders the curve + the v46 risk block (drawdown/vol/sharpe), the web scope picker lists it, and the cadence gate is independent of the real feed (different note tag — one feed being fresh never starves the other). Unpriceable tokens are **excluded from the total with a warning**, never guessed; an empty book is a no-op; `engine.snapshotIncludePaper: false` opts out. The promote decision finally has the numbers it needs: "the paper run's max drawdown and Sharpe over the last month" is one command.
...
```bash
tradekit incident --window 4h --out postmortem.md
```

One markdown document in reviewer order: **verdict** (the digest's health classification + reasons) → **activity** (trades/fires/failures, signals received vs *fired NOTHING*, alert transitions, the equity move) → **config changes in the window** (the most-asked postmortem question — "what changed before it broke", with `diff-version`/`rollback` pointers) → **operator/agent notes** (untagged notes survive `--strategy`, same global-context semantics as the timeline) → the **critical and warn event tails** (chronological inside each band so cause-and-effect reads top-down). Pure composition over the existing gatherers — the report can never disagree with the surfaces it summarizes. MCP `incident_report` tells investigating agents to read it FIRST: it says where to drill (`order replay`, `schedule replay`, `timeline`).
...
**Playbooks speak signal too (v37).** Order entries accept `"trigger": "signal", "signalName": "tv-breakout"` — the full TradingView strategy (signal entry + OCO brackets + DCA schedules) deploys as ONE playbook file, flips paper↔real with promote, and diffs/replaces like everything else (a `signalName` change classifies as recreate — it's trigger identity). `signalName` is deliberately NOT pb-namespaced: the external alert name is global, and several playbooks listening to one signal is a feature. Playbook *backtests* replay signal entries against **recorded signal history** — `backtest playbook ... --signals-from-history` answers "with the alerts I actually received, how would this strategy have done?": each entry fires at the first datapoint at-or-after a matching arrival in the signal inbox (the sim twin of "the next engine tick after the webhook"), stale-before-window arrivals never fire, and the on_fill bracket + OCO cascade simulate as usual. MCP `backtest_playbook` additionally accepts an inline `signals: [{name, at}]` array for *hypothetical* replays ("what if the alert had come a day earlier?"). Without history provided, signal entries stay rejected with a teaching message — no history to replay is a guess, not a simulation.
...
**Activation boundaries — `--start-at` / `--start-in` (v38).** Orders gain the activation window schedules always had: *"arm this breakout buy only after the Fed announcement at 18:00"*. Before `start_at` the engine ignores the order **entirely** — no trigger evaluation, no trailing watermark (pre-announcement chop must not set the HWM your stop will measure retracement from), and signals received during pre-start are **permanently ineligible** for that order (eligibility starts at `max(created_at, start_at)` — an alert that fired before you were armed predates your intent). Three boundaries deliberately still apply during pre-start: expiry retires the order (validity window ≠ activity window), OCO peers can cancel it, and the runway keeps reserving its one-shot spend (conservative). Expiry-before-start rejects at create. Works for every trigger type and in playbook order entries.
...
- **The queue is visible everywhere the operator already looks (v47.5)**: the **digest** counts proposals/decisions and pushes the verdict to `attention` while anything is pending (with the oldest age) or expired un-reviewed; **`tradekit doctor`** warns on an open queue ("the agent is blocked until you decide") and on recent un-reviewed expiries ("approval requests are being missed"); the **timeline** gains `intent.created` (warn while awaiting) and `intent.decided` events — expiry synthesizes a decided event at the deadline, so "EXPIRED un-reviewed" shows up in the forensic stream and the incident report; the **web Overview** shows a pending banner polling read-only `GET /api/intents`. An approval queue nobody sees is a silently blocked agent — visibility is part of the feature.
