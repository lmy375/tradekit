...
**Playbooks speak signal too (v37).** Order entries accept `"trigger": "signal", "signalName": "tv-breakout"` — the full TradingView strategy (signal entry + OCO brackets + DCA schedules) deploys as ONE playbook file, flips paper↔real with promote, and diffs/replaces like everything else (a `signalName` change classifies as recreate — it's trigger identity). `signalName` is deliberately NOT pb-namespaced: the external alert name is global, and several playbooks listening to one signal is a feature. Playbook *backtests* reject signal entries with a teaching message (no signal history to replay).
